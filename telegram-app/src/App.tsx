  ...
  const authenticateUser = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log('вњ… User authenticated successfully')
      } else {
        const text = await response.text().catch(() => '')
        console.error('вќЊ Authentication failed:', text)
      ...
      if (error?.name === 'AbortError') {
        ...
        console.error('вќЊ Authentication error:', error)
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.status === 'COMPLETED') {
            toast.dismiss()
            toast.success('вњ… Payment successful! Your balance has been updated.')
            await refreshData()
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (deposit && deposit.status === 'COMPLETED') {
          toast.success('вњ… Payment received! Your balance has been updated.')
          await refreshData()
          await fetchTransactions()
          setDepositPaymentUrl('')
          return true
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.status === 'PENDING') {
          toast.success('вЏі Your withdrawal request has been submitted and is awaiting approval. We will notify you shortly!')
        } else if (data.status === 'PROCESSING') {
          toast.success('вЏі Your withdrawal is currently being processed. You will receive a notification once it is complete.')
        } else if (data.status === 'COMPLETED') {
          toast.success('вњ… Withdrawal completed successfully!')
        } else {
        ...
        <div className="max-w-2xl mx-auto">
          ...
          {activeTab === 'trade' && userData?.arbitrageTradeEnabled && (
            <TradeTab
              ...
              priceCheckLimit={Math.max(1, Number(userData?.tradePriceCheckLimit || 1))}
              currentBalance={Math.max(0, Number(userProfile.balance || 0))}
              telegramId={telegramUserId}
              authToken={authToken}
            />
          )}
              ...
              <div className="bg-accent/20 border border-accent rounded-lg p-4 sm:p-6 space-y-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center gap-2 px-3 py-1 bg-accent/30 rounded-full">
                    ...
                    <span className="text-xs sm:text-sm text-foreground">В·</span>
            ...
            <div className="space-y-6 pb-4">
              ...
              <div className="grid grid-cols-1 gap-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <Button 
                    className="w-full bg-transparent border-2 border-accent text-accent hover:bg-accent/10 font-bold py-6 text-base uppercase"
                    onClick={() => window.open('https://website.syntrix.uno', '_blank')}
                  >
                    {t.website}
                  </Button>
                </div>
